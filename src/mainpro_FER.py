...
if opt.resume:
    ...
    checkpoint = torch.load(os.path.join(path,'FER2013_PrivateTest_model.t7'))
...
def PublicTest(epoch):
    ...
    if PublicTest_acc > best_PublicTest_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path,'FER2013_PublicTest_model.t7'))
...
def PrivateTest(epoch):
    ...
    if PrivateTest_acc > best_PrivateTest_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path,'FER2013_PrivateTest_model.t7'))
        best_PrivateTest_acc = PrivateTest_acc
        best_PrivateTest_acc_epoch = epoch
